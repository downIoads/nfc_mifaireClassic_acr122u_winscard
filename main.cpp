...
const BYTE APDU_Firmware[] = { 0xff, 0x00, 0x48, 0x00, 0x00 };
const BYTE APDU_LoadDefaultKey[] = { 0xff, 0x82, 0x00, 0x00, 0x06, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff };
const BYTE APDU_Authenticate_0_A[] = { 0xff, 0x86, 0x00, 0x00, 0x05, 0x01, 0x00, 0x00, 0x60, 0x00 };
const BYTE APDU_Read_0[] = { 0xff, 0xB0, 0x00, 0x00, 0x10 };
...
void main(void)
{
	...
	{
		cbBuffer = 10;
		if (SendRecvReader(&hDual, APDU_Firmware, sizeof(APDU_Firmware), Buffer, &cbBuffer))
		{
		...
		cbBuffer = 2;
		if (SendRecvReader(&hDual, APDU_Authenticate_0_A, sizeof(APDU_Authenticate_0_A), Buffer, &cbBuffer))
		{
			wprintf(L"OK\n");
		}

		cbBuffer = 2 + 16;
		if (SendRecvReader(&hDual, APDU_Read_0, sizeof(APDU_Read_0), Buffer, &cbBuffer))
		{
			wprintf(L"OK\n");
		}
